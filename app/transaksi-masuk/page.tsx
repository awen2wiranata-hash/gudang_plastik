        ...
        <form onSubmit={handleSubmit}>
          <div className="flex gap-4 mb-6 pb-6 border-b border-gray-200">
            ...
            <div className="flex-1">
              ...
              <input type="text" required value={nomorNota} onChange={(e) => setNomorNota(e.target.value)} className="w-full border border-gray-300 rounded-md p-2 h-[38px]" placeholder="Contoh: INV-001" />
            </div>
          ...
          {keranjang.map((item, index) => (
            <div key={index} className="flex gap-4 mb-3 items-center">
              ...
              <div className="w-10">
                {index > 0 && (
                  <button type="button" onClick={() => hapusBaris(index)} className="w-full bg-red-100 hover:bg-red-200 text-red-600 font-bold p-2 h-[38px] rounded-md transition-colors" title="Hapus Baris">X</button>
                )}
              </div>
            </div>
          ))}
          ...
          <div className="flex justify-between mt-6">
            <button type="button" onClick={tambahBaris} className="text-blue-600 border border-blue-600 hover:bg-blue-50 font-medium py-2 px-4 rounded-md transition-colors">+ Tambah Baris Barang</button>
            <button type="submit" className="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-8 rounded-md transition-colors shadow-sm">Simpan Transaksi</button>
          </div>
        </form>
      </div>
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Rincian Barang</th>
            </tr>
          </thead>
          <tbody className="bg-white divide-y divide-gray-200">
            {loading ? <tr><td colSpan={3} className="text-center py-4">Memuat data...</td></tr> : riwayatMasuk.map((riwayat) => (
                ...
                <td className="px-6 py-4 text-sm text-gray-600">
                  <ul className="list-disc pl-4">
                    {riwayat.detailBarang.map((det, idx) => (
                      <li key={idx}><span className="font-medium">{det.barang.namaBarang}</span> : +{det.jumlah}</li>
                    ))}
                  </ul>
                </td>
              </tr>
